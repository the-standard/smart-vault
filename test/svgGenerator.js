...
const getSvgMintConfig = (minted, maxMintable, totalCollateralValue) => {
    return {
        vaultAddress: "0x1234567890123456789012345678901234567890",
        minted: ethers.utils.parseUnits(minted, 18),
        maxMintable: ethers.utils.parseUnits(maxMintable, 18),
        totalCollateralValue: ethers.utils.parseUnits(totalCollateralValue, 18),
        collateral: [],
        liquidated: false,
        version: 1,
...
describe('SVG Generator', async () => {
    ...
    it('should create an svg', async function () {
        const config = getSvgMintConfig("2500", "1000", "10000");
    ...
    it('should work with different configurations', async function () {
        const config = getSvgMintConfig("100", "50", "2000");
        const svg = await svgGenerator.generateSvg(1234, config);
        const svgDataUrl = `data:image/svg+xml;base64,${Buffer.from(svg).toString('base64')}`;
        expect(svgDataUrl).to.not.be.undefined;
    });
