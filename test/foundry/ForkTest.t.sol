...
contract ForkTest is ForkFixture {
    ...
    function test_autoRedemption() public {
        ...
        ISwapRouter(UNISWAP_ROUTER_ADDRESS).exactInputSingle(ISwapRouter.ExactInputSingleParams({
        ...
        bytes memory ethUSDsSwapPath = abi.encodePacked(WETH_ADDRESS, UNISWAP_FEE, USDC_ADDRESS, UNISWAP_FEE, address(usds));
        vm.prank(VAULT_MANAGER_OWNER);
        uint256 ETHToSell = ethCollateral / 100;
        uint256 _USDsRedeemed = smartVaultManager.vaultAutoRedemption(vaultTokenID, UNISWAP_ROUTER_ADDRESS, address(0), ethUSDsSwapPath, ETHToSell);
